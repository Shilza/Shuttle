...
test(`${SUITE_NAME} error Like does not exists (posts)`, async ({client, assert}) => {
...
test(`${SUITE_NAME} error Like does not exists (comments)`, async ({client, assert}) => {
...
test(`${SUITE_NAME} successful (comment)`, async ({client, assert}) => {
  ...
  assert.equal(response.body.message, "Comment unliked successfully");
  assert.isFalse(like);
});
...
test(`${SUITE_NAME} successful (post)`, async ({client, assert}) => {
  ...
  response.assertStatus(200);
  assert.equal(response.body.message, "Post unliked successfully");
  assert.isFalse(like);
});
