    ...
    async up() {
        await Database
            .raw(
                ...
                'FOR EACH ROW DELETE FROM actions WHERE receiver_id = (SELECT owner_id FROM posts WHERE id = OLD.post_id) AND initiator_id=OLD.owner_id AND type=2 AND entity_id=OLD.id'
            );
    }
